...
B = [e; f];
%--------------------------------------------------------------------------
timeStep = 0.1;     % time steps of 100ms for integration

totalTime = 40;     % total time for simulation

tspan = 0:timeStep:totalTime;   % build timepoint vector
%--------------------------------------------------------------------------

y0 = [pi; 0.1;];    % [pi = rod's angle] & [0.1 = rod's velocity]
[t,y] = ode45(@(t,y)THE_X_DOT_FUNCTION(A, y, B, 0),tspan,y0);
...
%--------------------------------------------------------------------------
% get variables
x  = y(:, 1)*0;    % cart positon [0 = not moving]
...
% animate pendulum
figure
AnimatePendulumCart(th, x, L/2, tspan, range, kickFlag, "PENDULUM");
